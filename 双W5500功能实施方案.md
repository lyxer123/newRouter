...
2. **新增W5500卡2**：
   - 与卡1共享SPI总线（SCK、MOSI、MISO）
   - 独立控制引脚：CS2、INT2、RST2
3. **角色分配策略**：
...
### 1.2 技术难点
1. **硬件层面**：
   - 两个W5500共享SPI总线，需要独立的CS片选信号
   - 需要独立的INT中断信号和RST复位信号
   - SPI总线需要支持多设备复用
...
### 2.2 关键配置项
- `CONFIG_BRIDGE_ETH_SPI_CS0_GPIO`：卡1的CS引脚
- `CONFIG_BRIDGE_ETH_SPI_INT0_GPIO`：卡1的INT引脚
- `CONFIG_BRIDGE_ETH_SPI_PHY_RST0_GPIO`：卡1的RST引脚
...
### 步骤1：硬件配置扩展
**目标**：在Kconfig中添加W5500卡2的GPIO配置项

**实现内容**：
1. 在`Kconfig`中添加卡2的配置项：
   - `CONFIG_BRIDGE_ETH_SPI_CS1_GPIO`：卡2的CS引脚
   - `CONFIG_BRIDGE_ETH_SPI_INT1_GPIO`：卡2的INT引脚
   - `CONFIG_BRIDGE_ETH_SPI_PHY_RST1_GPIO`：卡2的RST引脚
   - `CONFIG_BRIDGE_ETH_SPI_PHY_ADDR1`：卡2的PHY地址
...
### 步骤2：扩展W5500初始化函数
**目标**：修改`bridge_eth.c`，支持初始化第二个W5500

**实现内容**：
1. 创建新的初始化函数`esp_spi_eth_new_w5500_card2`，用于初始化卡2
2. 修改`esp_bridge_eth_spi_init`函数，支持初始化多个W5500设备
3. 为每个W5500设备创建独立的`eth_handle`
4. 确保SPI总线共享，但CS、INT、RST独立控制

**关键代码修改点**：
- `esp_spi_eth_new_w5500`函数：添加设备索引参数
- SPI设备配置：为卡2创建独立的`spi_device_interface_config_t`
- 使用独立的CS引脚配置
