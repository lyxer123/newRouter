...
**检验步骤**：
- 连接卡1和4G，验证流量优先走卡1
- 断开卡1网线，验证流量自动切换到WiFi Station
- 断开WiFi Station，验证流量自动切换到4G
- 重新连接卡1，验证流量自动切回卡1
...
- 验证下位设备连接到卡1（作为LAN）、卡2或WiFi热点时，都能获取到192.168.4.x范围内的IP地址
- 验证下位设备能够通过卡1（作为WAN）、WiFi Station或4G访问互联网
...
### 5.4 bridge_wifi.c修改要点
1. 确保WiFi Station接口正确创建和配置
2. 修改DNS更新逻辑，支持WiFi Station作为WAN
3. 添加WiFi Station状态监控
