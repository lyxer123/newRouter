...
### 2.1 目标
实现商用路由器功能，WiFi热点使用独立的IP子网192.168.5.x，而卡2和卡1（作为LAN时）共享另一个IP子网192.168.4.x，卡1（作为WAN时）、WiFi Station和4G模块使用各自获取的IP地址。
...
### 2.4 DHCP地址池
- **WiFi热点地址池**：192.168.5.100-192.168.5.200
- **卡2和卡1作为LAN时地址池**：192.168.4.100-192.168.4.200
- **分配策略**：
  ...
  - WiFi热点始终启用DHCP服务器
  - 通过协调机制避免地址冲突
  - WiFi热点使用网关地址(192.168.5.1)
  - 卡2和卡1作为LAN时使用网关地址(192.168.4.1)
...
#### 1.1 实现目标
在Kconfig配置系统中添加W5500卡2的GPIO配置项，支持配置卡2的CS、INT、RST引脚（共享SPI总线方案）。
...
卡2的配置项已经在现有Kconfig中定义，确保以下配置项存在：
...
#### 2.1 实现目标
修改`bridge_eth.c`，支持初始化第二个W5500设备（共享SPI总线方案）。
...
**修改WiFi SoftAP的IP地址分配**：
```c
// WiFi SoftAP使用独立的192.168.5.x网段
esp_netif_ip_info_t softap_ip_info = {0};
softap_ip_info.ip.addr = ESP_IP4TOADDR(192, 168, 5, 1);
softap_ip_info.gw.addr = ESP_IP4TOADDR(192, 168, 5, 1);
softap_ip_info.netmask.addr = ESP_IP4TOADDR(255, 255, 255, 0);
esp_netif_set_ip_info(softap_netif, &softap_ip_info);
```
...
if (enable_dhcps) {
    ...
    // 根据接口类型设置不同的地址池
    if (is_wifi_softap) {
        // WiFi SoftAP使用192.168.5.x地址池
        lease.start_ip.addr = ESP_IP4TOADDR(192, 168, 5, 100);
        lease.end_ip.addr = ESP_IP4TOADDR(192, 168, 5, 200);
    } else {
        // 以太网卡使用192.168.4.x地址池
        lease.start_ip.addr = ESP_IP4TOADDR(192, 168, 4, 100);
        lease.end_ip.addr = ESP_IP4TOADDR(192, 168, 4, 200);
    }
    esp_netif_dhcps_option(netif, ESP_NETIF_OP_SET, ESP_NETIF_REQUESTED_IP_ADDRESS, &lease, sizeof(dhcps_lease_t));
...
**修改`esp_bridge_create_all_netif`函数以确保正确的IP分配**：
```c
void esp_bridge_create_all_netif(void)
{
    ESP_LOGI(TAG, "esp-iot-bridge version: %d.%d.%d", IOT_BRIDGE_VER_MAJOR, IOT_BRIDGE_VER_MINOR, IOT_BRIDGE_VER_PATCH);
    ...
    #if defined(CONFIG_BRIDGE_DATA_FORWARDING_NETIF_ETHERNET)
        // 卡2固定为LAN，始终启用DHCP服务器
        // 为卡2预设IP信息以确保使用192.168.4.1网关
        esp_netif_ip_info_t card2_ip_info = {0};
        card2_ip_info.ip.addr = ESP_IP4TOADDR(192, 168, 4, 1);
        card2_ip_info.gw.addr = ESP_IP4TOADDR(192, 168, 4, 1);
        card2_ip_info.netmask.addr = ESP_IP4TOADDR(255, 255, 255, 0);
        esp_bridge_create_eth_netif(&card2_ip_info, NULL, true, true, 1);  // card_index = 1
    #endif
...
**注意**：WiFi SoftAP接口应使用独立的IP子网192.168.5.x，网关地址为192.168.5.1，与以太网卡的192.168.4.x子网完全分离。
...
- [ ] 测试卡2固定LAN：
  ...
  - [ ] 验证卡2不会因为网络状态变化而改变角色
- [ ] IP地址池验证：
  - [ ] 验证卡1作为LAN时的IP地址池（192.168.4.100-192.168.4.200）
  - [ ] 验证卡2的IP地址池（192.168.4.100-192.168.4.200）
  - [ ] 验证WiFi热点的IP地址池（192.168.5.100-192.168.5.200）
  - [ ] 确保WiFi热点使用独立子网（192.168.5.x）
- [ ] 代码检查：
