                                 ...
                                 int card_index)  // 新增参数
{
    ...
    if (card_index == 0) {
        w5500_config.int_gpio_num = CONFIG_BRIDGE_ETH_SPI_INT0_CARD1_GPIO;
        phy_config_spi.phy_addr = CONFIG_BRIDGE_ETH_SPI_PHY_ADDR0_CARD1;
        phy_config_spi.reset_gpio_num = CONFIG_BRIDGE_ETH_SPI_PHY_RST0_CARD1_GPIO;
    } else if (card_index == 1) {
        w5500_config.int_gpio_num = CONFIG_BRIDGE_ETH_SPI_INT0_CARD2_GPIO;
        phy_config_spi.phy_addr = CONFIG_BRIDGE_ETH_SPI_PHY_ADDR0_CARD2;
        phy_config_spi.reset_gpio_num = CONFIG_BRIDGE_ETH_SPI_PHY_RST0_CARD2_GPIO;
    }
...
esp_err_t esp_bridge_eth_spi_init(esp_netif_t* eth_netif_spi, int card_index)
{
    ...
    // 根据card_index选择不同的SPI主机和GPIO
    spi_host_device_t spi_host;
    if (card_index == 0) {
        spi_host = CONFIG_BRIDGE_ETH_SPI_HOST_CARD1;
    } else if (card_index == 1) {
        spi_host = CONFIG_BRIDGE_ETH_SPI_HOST_CARD2;
    } else {
    ...
    spi_bus_config_t buscfg = {
        .miso_io_num = (card_index == 0) ? CONFIG_BRIDGE_ETH_SPI_MISO_CARD1_GPIO : CONFIG_BRIDGE_ETH_SPI_MISO_CARD2_GPIO,
        .mosi_io_num = (card_index == 0) ? CONFIG_BRIDGE_ETH_SPI_MOSI_CARD1_GPIO : CONFIG_BRIDGE_ETH_SPI_MOSI_CARD2_GPIO,
        .sclk_io_num = (card_index == 0) ? CONFIG_BRIDGE_ETH_SPI_SCLK_CARD1_GPIO : CONFIG_BRIDGE_ETH_SPI_SCLK_CARD2_GPIO,
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
    };
    
    // Install GPIO ISR handler (每个设备独立初始化)
    gpio_install_isr_service(0);
    
    // Init SPI bus (每个设备独立初始化)
    ...
    spi_device_interface_config_t devcfg = {
        ...
        .queue_size = 20,
        .spics_io_num = (card_index == 0) ? CONFIG_BRIDGE_ETH_SPI_CS0_CARD1_GPIO : CONFIG_BRIDGE_ETH_SPI_CS0_CARD2_GPIO
    };
