...
在`BRIDGE_USE_SPI_ETHERNET`配置块中，添加卡2的控制信号配置（共享SPI总线）：

**注意**：卡1的配置项已经存在，只需确认或修改以下值：
- `CONFIG_BRIDGE_ETH_SPI_SCLK_GPIO`：GPIO9（共享）
- `CONFIG_BRIDGE_ETH_SPI_MOSI_GPIO`：GPIO3（共享）
- `CONFIG_BRIDGE_ETH_SPI_MISO_GPIO`：GPIO46（共享）
- `CONFIG_BRIDGE_ETH_SPI_CS0_GPIO`：GPIO10（卡1）
- `CONFIG_BRIDGE_ETH_SPI_INT0_GPIO`：GPIO12（卡1）
- `CONFIG_BRIDGE_ETH_SPI_PHY_RST0_GPIO`：GPIO38（卡1）
...
config BRIDGE_ETH_SPI_CS1_GPIO
    ...
    help
        ...
        Note: Card1 and Card2 share the same SPI bus (SCK=GPIO9, MOSI=GPIO3, MISO=GPIO46).
...
1. **配置验证**：
   ...
   - 验证卡1的配置项（共享SPI总线）：
     - `SPI Host`（例如SPI2）
     - `SPI SCLK GPIO`（应为GPIO9）
     - `SPI MOSI GPIO`（应为GPIO3）
     - `SPI MISO GPIO`（应为GPIO46）
     - `SPI CS0 GPIO`（应为GPIO10，卡1）
     - `Interrupt GPIO number SPI Ethernet module #1`（应为GPIO12）
     - `PHY Reset GPIO number of SPI Ethernet Module #1`（应为GPIO38）
   - 验证卡2的配置项：
     - `SPI CS1 GPIO number for SPI Ethernet module #2`（应为GPIO11）
     - `Interrupt GPIO number SPI Ethernet module #2`（应为GPIO13）
     - `PHY Reset GPIO number of SPI Ethernet Module #2`（应为GPIO34）
     - `PHY Address of SPI Ethernet Module #2`（应为1）
...
2. **运行时验证**：
   - 查看串口日志，应该看到SPI总线初始化信息（只初始化一次）
   - 检查日志中是否有"Ethernet Started"消息（应该出现两次，卡1和卡2）
   - 检查是否有GPIO配置错误
   - 验证两个W5500都成功初始化

3. **硬件验证**（共享SPI）：
   - 使用示波器检查共享SPI总线信号（SCK=GPIO9, MOSI=GPIO3, MISO=GPIO46）
   - 使用示波器检查CS0（GPIO10）和CS1（GPIO11）信号是否正确切换
   - 验证同一时间只有一个CS信号有效（ESP-IDF SPI驱动自动切换）
   - 检查INT0（GPIO12）和INT1（GPIO13）信号是否正确连接
   - 检查RST0（GPIO38）和RST1（GPIO34）信号（如果启用）
...
### 6.5.1 实现目标
配置卡1、卡2和WiFi热点共享同一个LAN子网（192.168.4.x），而卡1作为WAN时使用不同的子网（如192.168.1.x）。

### 6.5.2 具体实现
**文件1**：`managed_components/espressif__iot_bridge/src/bridge_common.c`
