...
// 新增支持双网卡的W5500初始化函数
esp_err_t esp_spi_eth_new_w5500_dual(spi_device_interface_config_t *spi_devcfg, 
...
typedef esp_err_t (*esp_bridge_spi_eth_module_dual_t)(spi_device_interface_config_t *spi_devcfg, esp_netif_t *eth_netif_spi, esp_eth_handle_t *eth_handle_spi, int card_index);
...
// 新增支持双网卡的SPI初始化函数
esp_err_t esp_bridge_eth_spi_init_dual(esp_netif_t* eth_netif_spi, int card_index)
{
    esp_err_t ret = ESP_FAIL;
    static bool eth_is_start = false;
    ...
    // 初始化对应的W5500设备
    esp_bridge_spi_eth_module_dual_t esp_bridge_spi_eth_module_dual[] = {
#if CONFIG_ETH_SPI_ETHERNET_W5500
        esp_spi_eth_new_w5500_dual,
#endif
    };
    
    uint8_t spi_eth_module_max = sizeof(esp_bridge_spi_eth_module_dual)/sizeof(esp_bridge_spi_eth_module_dual[0]);
    for (uint8_t i = 0; i < spi_eth_module_max; i++) {
        if (esp_bridge_spi_eth_module_dual[i](&devcfg, eth_netif_spi, eth_handle_spi, card_index) == ESP_OK) {
            break;
        }
    }
                                          ...
                                          int card_index)
{
    ...
    // 使用双网卡专用的网络接口创建函数
    esp_netif_t* netif = esp_bridge_create_netif_dual(&eth_config, ip_info, mac, enable_dhcps, card_index);
    if (netif) {
        // 卡1：使用自动WAN/LAN切换
        if (card_index == 0) {
#if defined(CONFIG_BRIDGE_NETIF_ETHERNET_AUTO_WAN_OR_LAN)
        ...
        esp_bridge_eth_init(netif);
#elif CONFIG_BRIDGE_USE_SPI_ETHERNET
        // 共享SPI方案：根据card_index初始化不同的W5500设备
        esp_bridge_eth_spi_init_dual(netif, card_index);
#endif
        esp_netif_up(netif);
